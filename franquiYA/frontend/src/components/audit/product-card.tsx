...
export function ProductCard({ product, onConfirm, disabled }: ProductCardProps) {
  ...
  if (confirmed) {
    return (
      <Card className="border-2 border-emerald-500/50 bg-emerald-500/10 transition-all duration-300">
  ...
  return (
    <Card className={cn(
      'bg-[#1a1a1a] border border-white/5 transition-all duration-200',
      disabled && 'opacity-50'
    )}>
      <CardContent className="p-4">
        <div className="flex gap-4">
          <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-xl bg-white/5">
          ...
          <div className="flex-1">
            <div className="mb-2 flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Sistema: <span className="font-mono text-gray-300">{product.current_stock}</span>
                </p>
              </div>
              {diff !== 0 && total > 0 && (
                <span className={cn(
            ...
            <button
              ...
              className={cn(
                'mt-3 w-full rounded-xl py-3 text-base font-semibold transition-all duration-200',
                closedQty > 0 || openQty > 0
                  ? 'bg-[#E31D2B] text-white hover:bg-[#C41925] shadow-lg shadow-[#E31D2B]/30 hover:shadow-[#E31D2B]/50'
